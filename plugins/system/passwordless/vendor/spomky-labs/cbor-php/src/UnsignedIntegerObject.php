...
final class UnsignedIntegerObject extends \Akeeba\Passwordless\CBOR\AbstractAkeeba\Passwordless\CBORObject implements \Akeeba\Passwordless\CBOR\Normalizable
{
    ...
    {
        $integer = \Akeeba\Passwordless\Brick\Math\BigInteger::fromBase($value, 16);
    ...
    public function getValue(): string
    {
        ...
        $integer = \Akeeba\Passwordless\Brick\Math\BigInteger::fromBase(bin2hex($this->data), 16);
    ...
    {
        if ($integer->isLessThan(\Akeeba\Passwordless\Brick\Math\BigInteger::zero())) {
            throw new InvalidArgumentException('The value must be a positive integer.');
        }

        switch (true) {
            case $integer->isLessThan(\Akeeba\Passwordless\Brick\Math\BigInteger::of(24)):
                $ai = $integer->toInt();
                $data = null;
                break;
            case $integer->isLessThan(\Akeeba\Passwordless\Brick\Math\BigInteger::fromBase('FF', 16)):
                $ai = 24;
                $data = self::hex2bin(str_pad($integer->toBase(16), 2, '0', STR_PAD_LEFT));
                break;
            case $integer->isLessThan(\Akeeba\Passwordless\Brick\Math\BigInteger::fromBase('FFFF', 16)):
                $ai = 25;
                $data = self::hex2bin(str_pad($integer->toBase(16), 4, '0', STR_PAD_LEFT));
                break;
            case $integer->isLessThan(\Akeeba\Passwordless\Brick\Math\BigInteger::fromBase('FFFFFFFF', 16)):
                $ai = 26;
