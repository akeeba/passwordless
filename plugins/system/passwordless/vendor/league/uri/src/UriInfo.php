...
final class UriInfo
{
    ...
    {
        if ($uri instanceof Psr7UriInterface || $uri instanceof \Akeeba\Passwordless\League\Uri\Contracts\UriInterface) {
            return $uri;
        }
    ...
    private static function normalize($uri)
    {
        ...
        if ('/' === ($path[0] ?? '') || '' !== $uri->getScheme().$uri->getAuthority()) {
            $path = \Akeeba\Passwordless\League\Uri\UriResolver::resolve($uri, $uri->withPath('')->withQuery($null))->getPath();
        }
        ...
        return $uri
            ->withHost(\Akeeba\Passwordless\League\Uri\Uri::createFromComponents(['host' => $uri->getHost()])->getHost())
            ->withPath($path)
    ...
    public static function getOrigin($uri): ?string
    {
        ...
        if ('blob' === $scheme) {
            $uri = \Akeeba\Passwordless\League\Uri\Uri::createFromString($uri->getPath());
