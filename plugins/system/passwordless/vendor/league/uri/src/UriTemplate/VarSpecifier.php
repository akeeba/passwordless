...
final class VarSpecifier
{
    ...
    public static function createFromString(string $specification): self
    {
        if (1 !== preg_match(self::REGEXP_VARSPEC, $specification, $parsed)) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError('The variable specification "'.$specification.'" is invalid.');
        }
        ...
        if (10000 <= $parsed['position']) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError('The variable specification "'.$specification.'" is invalid the position modifier must be lower than 10000.');
        }
