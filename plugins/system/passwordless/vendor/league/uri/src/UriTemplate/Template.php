...
final class Template
{
    ...
    private function __construct(string $template, \Akeeba\Passwordless\League\Uri\UriTemplate\Expression ...$expressions)
    {
    ...
    public static function createFromString($template): self
    {
        ...
        if (false !== strpos($remainder, '{') || false !== strpos($remainder, '}')) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError('The template "'.$template.'" contains invalid expressions.');
        }
        ...
        foreach ($findings as $finding) {
            if (!isset($names[$finding[0]])) {
                $arguments[] = \Akeeba\Passwordless\League\Uri\UriTemplate\Expression::createFromString($finding[0]);
                $names[$finding[0]] = 1;
            }
        }
