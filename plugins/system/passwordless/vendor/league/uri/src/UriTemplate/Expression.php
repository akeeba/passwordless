...
namespace Akeeba\Passwordless\League\Uri\UriTemplate;

use Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError;
use Akeeba\Passwordless\League\Uri\Exceptions\TemplateCanNotBeExpanded;
...
final class Expression
{
    ...
    private function __construct(string $operator, \Akeeba\Passwordless\League\Uri\UriTemplate\VarSpecifier ...$varSpecifiers)
    {
    ...
    {
        $mapper = static function (\Akeeba\Passwordless\League\Uri\UriTemplate\VarSpecifier $varSpecifier): string {
    ...
    {
        $mapper = static function (\Akeeba\Passwordless\League\Uri\UriTemplate\VarSpecifier $variable): string {
    ...
    public static function createFromString(string $expression): self
    {
        if (1 !== preg_match(self::REGEXP_EXPRESSION, $expression, $parts)) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError('The expression "'.$expression.'" is invalid.');
        }
        ...
        if ('' !== $parts['operator'] && false !== strpos(self::RESERVED_OPERATOR, $parts['operator'])) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError('The operator used in the expression "'.$expression.'" is reserved.');
        }

        $mapper = static function (string $varSpec): \Akeeba\Passwordless\League\Uri\UriTemplate\VarSpecifier {
            return \Akeeba\Passwordless\League\Uri\UriTemplate\VarSpecifier::createFromString($varSpec);
        };

        return new \Akeeba\Passwordless\League\Uri\UriTemplate\Expression($parts['operator'], ...array_map($mapper, explode(',', $parts['variables'])));
    }
    ...
    public function expand(\Akeeba\Passwordless\League\Uri\UriTemplate\VariableBag $variables): string
    {
        $parts = [];
     ...
     */
    private function replace(\Akeeba\Passwordless\League\Uri\UriTemplate\VarSpecifier $varSpec, \Akeeba\Passwordless\League\Uri\UriTemplate\VariableBag $variables): string
    {
     ...
     */
    private function inject($value, \Akeeba\Passwordless\League\Uri\UriTemplate\VarSpecifier $varSpec, bool $useQuery): array
    {
        if (is_string($value)) {
     ...
     * @return array{0:string, 1:bool}
     */
    private function replaceString(string $value, \Akeeba\Passwordless\League\Uri\UriTemplate\VarSpecifier $varSpec, bool $useQuery): array
    {
        if (':' === $varSpec->modifier()) {
     ...
     * @return array{0:string, 1:bool}
     */
    private function replaceList(array $value, \Akeeba\Passwordless\League\Uri\UriTemplate\VarSpecifier $varSpec, bool $useQuery): array
    {
        if ([] === $value) {
            return ['', false];
        }
        ...
        if (':' === $varSpec->modifier()) {
            throw \Akeeba\Passwordless\League\Uri\Exceptions\TemplateCanNotBeExpanded::dueToUnableToProcessValueListWithPrefix($varSpec->name());
        }
