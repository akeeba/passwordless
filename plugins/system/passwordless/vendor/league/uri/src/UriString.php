...
final class UriString
{
    ...
    public static function parse($uri): array
    {
        ...
        if (1 === preg_match(self::REGEXP_INVALID_URI_CHARS, $uri)) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('The uri `%s` contains invalid characters', $uri));
        }
        ...
        if (':' === $parts['scheme'] || 1 !== preg_match(self::REGEXP_URI_SCHEME, $parts['scontent'])) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('The uri `%s` contains an invalid scheme', $uri));
        }

        if ('' === $parts['scheme'].$parts['authority'] && 1 === preg_match(self::REGEXP_INVALID_PATH, $parts['path'])) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('The uri `%s` contains an invalid path.', $uri));
        }
    ...
    private static function filterHost(string $host): string
    {
        ...
        if (!self::isIpHost(substr($host, 1, -1))) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('Host `%s` is invalid : the IP host is malformed', $host));
        }
    ...
    private static function filterRegisteredName(string $host): string
    {
        ...
        if (1 !== preg_match(self::REGEXP_IDN_PATTERN, $formatted_host)) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('Host `%s` is invalid : the host is not a valid registered name', $host));
        }
