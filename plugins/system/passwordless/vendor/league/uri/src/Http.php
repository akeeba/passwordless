...
final class Http implements Psr7UriInterface, \JsonSerializable
{
     ...
     * New instance.
     */
    private function __construct(\Akeeba\Passwordless\League\Uri\Contracts\UriInterface $uri)
    {
        $this->validate($uri);
        $this->uri = $uri;
    }
     ...
     */
    private function validate(\Akeeba\Passwordless\League\Uri\Contracts\UriInterface $uri): void
    {
        $scheme = $uri->getScheme();
        if (null === $scheme && '' === $uri->getHost()) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('an URI without scheme can not contains a empty host string according to PSR-7: %s', (string) $uri));
        }

        $port = $uri->getPort();
        if (null !== $port && ($port < 0 || $port > 65535)) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('The URI port is outside the established TCP and UDP port ranges: %s', (string) $uri->getPort()));
        }
    }
    ...
    {
        return new self(\Akeeba\Passwordless\League\Uri\Uri::createFromString($uri));
    }
    ...
    {
        return new self(\Akeeba\Passwordless\League\Uri\Uri::createFromComponents($components));
    }
    ...
    {
        return new self(\Akeeba\Passwordless\League\Uri\Uri::createFromServer($server));
    }
    ...
    {
        return new self(\Akeeba\Passwordless\League\Uri\Uri::createFromBaseUri($uri, $base_uri));
    }
    ...
    public static function createFromUri($uri): self
    {
        ...
        return new self(\Akeeba\Passwordless\League\Uri\Uri::createFromUri($uri));
    }
