...
namespace Akeeba\Passwordless\League\Uri;

use Akeeba\Passwordless\League\Uri\Contracts\UriInterface;
use Akeeba\Passwordless\League\Uri\Exceptions\FileinfoSupportMissing;
use Akeeba\Passwordless\League\Uri\Exceptions\IdnaConversionFailed;
use Akeeba\Passwordless\League\Uri\Exceptions\IdnSupportMissing;
use Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError;
use Akeeba\Passwordless\League\Uri\Idna\Idna;
...
final class Uri implements \Akeeba\Passwordless\League\Uri\Contracts\UriInterface
{
    /**
    ...
    private function formatScheme(?string $scheme): ?string
    {
        ...
        throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('The scheme `%s` is invalid.', $scheme));
    }
    ...
    private function formatUserInfo(?string $user, ?string $password): ?string
    {
        ...
        $user = preg_replace_callback($user_pattern, [\Akeeba\Passwordless\League\Uri\Uri::class, 'urlEncodeMatch'], $user);
        if (null === $password) {
            return $user;
        }
        ...
        return $user.':'.preg_replace_callback($password_pattern, [\Akeeba\Passwordless\League\Uri\Uri::class, 'urlEncodeMatch'], $password);
    }
    ...
    private function formatRegisteredName(string $host): string
    {
        ...
        if (1 === preg_match(self::REGEXP_HOST_GEN_DELIMS, $formatted_host)) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('The host `%s` is invalid : a registered name can not contain URI delimiters or spaces', $host));
        }

        $info = \Akeeba\Passwordless\League\Uri\Idna\Idna::toAscii($host, \Akeeba\Passwordless\League\Uri\Idna\Idna::IDNA2008_ASCII);
        if (0 !== $info->errors()) {
            throw \Akeeba\Passwordless\League\Uri\Exceptions\IdnaConversionFailed::dueToIDNAError($host, $info);
        }
    ...
    private function formatIp(string $host): string
    {
        ...
        $pos = strpos($ip, '%');
        if (false === $pos) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('The host `%s` is invalid : the IP host is malformed', $host));
        }
    ...
    private function formatPort($port = null): ?int
    {
        ...
        if (!is_int($port) && !(is_string($port) && 1 === preg_match('/^\d*$/', $port))) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('The port `%s` is invalid', $port));
        }
     ...
     */
    public static function createFromBaseUri($uri, $base_uri = null): \Akeeba\Passwordless\League\Uri\Contracts\UriInterface
    {
        if (!$uri instanceof \Akeeba\Passwordless\League\Uri\Contracts\UriInterface) {
            $uri = self::createFromString($uri);
        }

        if (null === $base_uri) {
            if (null === $uri->getScheme()) {
                throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('the URI `%s` must be absolute', (string) $uri));
            }
            ...
            $uri = \Akeeba\Passwordless\League\Uri\UriResolver::resolve($uri, $uri->withFragment(null)->withQuery(null)->withPath(''));

            return $uri;
        }

        if (!$base_uri instanceof \Akeeba\Passwordless\League\Uri\Contracts\UriInterface) {
            $base_uri = self::createFromString($base_uri);
        }

        if (null === $base_uri->getScheme()) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('the base URI `%s` must be absolute', (string) $base_uri));
        }

        /** @var UriInterface $uri */
        $uri = \Akeeba\Passwordless\League\Uri\UriResolver::resolve($uri, $base_uri);
    ...
    {
        $components = \Akeeba\Passwordless\League\Uri\UriString::parse($uri);
    ...
    public static function createFromDataPath(string $path, $context = null): self
    {
        ...
        if (!$finfo_support) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\FileinfoSupportMissing(sprintf('Please install ext/fileinfo to use the %s() method.', __METHOD__));
        }
        ...
        if (false === $raw) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('The file `%s` does not exist or is not readable', $path));
        }

        $mimetype = (string) (new \finfo(FILEINFO_MIME))->file(...$mime_args);

        return \Akeeba\Passwordless\League\Uri\Uri::createFromComponents([
            'scheme' => 'data',
    ...
    public static function createFromUnixPath(string $uri = ''): self
    {
        ...
        if ('/' !== ($uri[0] ?? '')) {
            return \Akeeba\Passwordless\League\Uri\Uri::createFromComponents(['path' => $uri]);
        }

        return \Akeeba\Passwordless\League\Uri\Uri::createFromComponents(['path' => $uri, 'scheme' => 'file', 'host' => '']);
    }
    ...
    public static function createFromWindowsPath(string $uri = ''): self
    {
        ...
        if ('' !== $root) {
            return \Akeeba\Passwordless\League\Uri\Uri::createFromComponents(['path' => '/'.$root.$uri, 'scheme' => 'file', 'host' => '']);
        }
        ...
        return \Akeeba\Passwordless\League\Uri\Uri::createFromComponents(['host' => $parts[0], 'path' => '/'.$parts[1], 'scheme' => 'file']);
    }
    ...
    public static function createFromUri($uri): self
    {
        ...
        if (!$uri instanceof Psr7UriInterface) {
            throw new \TypeError(sprintf('The object must implement the `%s` or the `%s`', Psr7UriInterface::class, \Akeeba\Passwordless\League\Uri\Contracts\UriInterface::class));
        }
    ...
    private static function fetchUserInfo(array $server): array
    {
        ...
        if (0 === strpos(strtolower($server['HTTP_AUTHORIZATION']), 'basic')) {
            ...
            if (false === $userinfo) {
                throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError('The user info could not be detected');
            }
    ...
    private static function fetchHostname(array $server): array
    {
        ...
        if (!isset($server['SERVER_ADDR'])) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError('The host could not be detected');
        }
    ...
    private function formatPath(string $path): string
    {
        ...
        $path = (string) preg_replace_callback($pattern, [\Akeeba\Passwordless\League\Uri\Uri::class, 'urlEncodeMatch'], $path);
    ...
    private function formatDataPath(string $path): string
    {
        ...
        if (strlen($path) !== strspn($path, self::ASCII) || false === strpos($path, ',')) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('The path `%s` is invalid according to RFC2937', $path));
        }
    ...
    private function assertValidPath(string $mimetype, string $parameters, string $data): void
    {
        if (1 !== preg_match(self::REGEXP_MIMETYPE, $mimetype)) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('The path mimetype `%s` is invalid', $mimetype));
        }
        ...
        if ([] !== $res) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('The path paremeters `%s` is invalid', $parameters));
        }
        ...
        if (false === $res || $data !== base64_encode($res)) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('The path data `%s` is invalid', $data));
        }
    }
    ...
    private function formatQueryAndFragment(?string $component): ?string
    {
        ...
        static $pattern = '/(?:[^'.self::REGEXP_CHARS_UNRESERVED.self::REGEXP_CHARS_SUBDELIM.'%:@\/\?]++|%(?![A-Fa-f0-9]{2}))/';
        return preg_replace_callback($pattern, [\Akeeba\Passwordless\League\Uri\Uri::class, 'urlEncodeMatch'], $component);
    }
    ...
    private function assertValidState(): void
    {
        if (null !== $this->authority && ('' !== $this->path && '/' !== $this->path[0])) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError('If an authority is present the path must be empty or start with a `/`.');
        }

        if (null === $this->authority && 0 === strpos($this->path, '//')) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('If there is no authority the path `%s` can not start with a `//`.', $this->path));
        }
        ...
        if (null === $this->authority
            ...
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError('In absence of a scheme and an authority the first path segment cannot contain a colon (":") character.');
        }
        ...
        throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('The uri `%s` is invalid for the `%s` scheme.', (string) $this, $this->scheme));
    }
     ...
     * {@inheritDoc}
     */
    public function withScheme($scheme): \Akeeba\Passwordless\League\Uri\Contracts\UriInterface
    {
    ...
    private function filterString($str): ?string
    {
        ...
        throw new \Akeeba\Passwordless\League\Uri\Exceptions\SyntaxError(sprintf('The component `%s` contains invalid characters.', $str));
    }

    /**
     * {@inheritDoc}
     */
    public function withUserInfo($user, $password = null): \Akeeba\Passwordless\League\Uri\Contracts\UriInterface
    {
        $user_info = null;
     ...
     * {@inheritDoc}
     */
    public function withHost($host): \Akeeba\Passwordless\League\Uri\Contracts\UriInterface
    {
     ...
     * {@inheritDoc}
     */
    public function withPort($port): \Akeeba\Passwordless\League\Uri\Contracts\UriInterface
    {
     ...
     * {@inheritDoc}
     */
    public function withPath($path): \Akeeba\Passwordless\League\Uri\Contracts\UriInterface
    {
     ...
     * {@inheritDoc}
     */
    public function withQuery($query): \Akeeba\Passwordless\League\Uri\Contracts\UriInterface
    {
     ...
     * {@inheritDoc}
     */
    public function withFragment($fragment): \Akeeba\Passwordless\League\Uri\Contracts\UriInterface
    {
