...
final class Idna
{
    ...
    private static function supportsIdna(): void
    {
        ...
        if (!$idnSupport) {
            throw new \Akeeba\Passwordless\League\Uri\Exceptions\IdnSupportMissing('IDN host can not be processed. Verify that ext/intl is installed for IDN support and that ICU is at least version 4.6.');
        }
    }
     ...
     */
    public static function toAscii(string $domain, int $options): \Akeeba\Passwordless\League\Uri\Idna\IdnaInfo
    {
        ...
        if (1 === preg_match(self::REGEXP_IDNA_PATTERN, $domain)) {
            ...
            if ([] === $idnaInfo) {
                return \Akeeba\Passwordless\League\Uri\Idna\IdnaInfo::fromIntl([
            ...
            return \Akeeba\Passwordless\League\Uri\Idna\IdnaInfo::fromIntl($idnaInfo);
        }
     ...
     */
    public static function toUnicode(string $domain, int $options): \Akeeba\Passwordless\League\Uri\Idna\IdnaInfo
    {
        $domain = rawurldecode($domain);

        if (false === stripos($domain, 'xn--')) {
            return \Akeeba\Passwordless\League\Uri\Idna\IdnaInfo::fromIntl(['result' => $domain, 'isTransitionalDifferent' => false, 'errors' => self::ERROR_NONE]);
        }
        ...
        if ([] === $idnaInfo) {
            throw \Akeeba\Passwordless\League\Uri\Exceptions\IdnaConversionFailed::dueToInvalidHost($domain);
        }
