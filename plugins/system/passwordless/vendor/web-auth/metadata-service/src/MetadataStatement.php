...
class MetadataStatement implements JsonSerializable
{
    ...
    public static function createFromArray(array $data): self
    {
        ...
        foreach (['description', 'protocolFamily'] as $key) {
            if (!isset($data[$key])) {
                throw new InvalidArgumentException(\Akeeba\Passwordless\Safe\sprintf('The parameter "%s" is missing', $key));
            }
        }
    ...
    public function jsonSerialize(): array
    {
        $data = [
            ...
            'tcDisplayPNGCharacteristics' => array_map(static function (\Akeeba\Passwordless\Webauthn\MetadataService\DisplayPNGCharacteristicsDescriptor $object): array {
                return $object->jsonSerialize();
            }, $this->tcDisplayPNGCharacteristics),
            'attestationRootCertificates' => $this->attestationRootCertificates,
            'ecdaaTrustAnchors' => array_map(static function (\Akeeba\Passwordless\Webauthn\MetadataService\EcdaaTrustAnchor $object): array {
                return $object->jsonSerialize();
            }, $this->ecdaaTrustAnchors),
            'icon' => $this->icon,
            'supportedExtensions' => array_map(static function (\Akeeba\Passwordless\Webauthn\MetadataService\ExtensionDescriptor $object): array {
                return $object->jsonSerialize();
