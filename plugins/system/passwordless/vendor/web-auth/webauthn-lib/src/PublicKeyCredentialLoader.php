...
class PublicKeyCredentialLoader
{
    ...
    public function loadArray(array $json): \Akeeba\Passwordless\Webauthn\PublicKeyCredential
    {
        ...
        try {
            foreach (['id', 'rawId', 'type'] as $key) {
                \Akeeba\Passwordless\Assert\Akeeba\Passwordless\Assertion::keyExists($json, $key, \Akeeba\Passwordless\Safe\sprintf('The parameter "%s" is missing', $key));
                \Akeeba\Passwordless\Assert\Akeeba\Passwordless\Assertion::string($json[$key], \Akeeba\Passwordless\Safe\sprintf('The parameter "%s" shall be a string', $key));
            }
            \Akeeba\Passwordless\Assert\Akeeba\Passwordless\Assertion::keyExists($json, 'response', 'The parameter "response" is missing');
            \Akeeba\Passwordless\Assert\Akeeba\Passwordless\Assertion::isArray($json['response'], 'The parameter "response" shall be an array');
            \Akeeba\Passwordless\Assert\Akeeba\Passwordless\Assertion::eq($json['type'], 'public-key', \Akeeba\Passwordless\Safe\sprintf('Unsupported type "%s"', $json['type']));

            $id = \Akeeba\Passwordless\Base64Url\Akeeba\Passwordless\Base64Url::decode($json['id']);
            $rawId = \Akeeba\Passwordless\Base64Url\Akeeba\Passwordless\Base64Url::decode($json['rawId']);
            \Akeeba\Passwordless\Assert\Akeeba\Passwordless\Assertion::true(hash_equals($id, $rawId));
    ...
    {
        \Akeeba\Passwordless\Assert\Akeeba\Passwordless\Assertion::keyExists($response, 'clientDataJSON', 'Invalid data. The parameter "clientDataJSON" is missing');
        \Akeeba\Passwordless\Assert\Akeeba\Passwordless\Assertion::string($response['clientDataJSON'], 'Invalid data. The parameter "clientDataJSON" is invalid');
        switch (true) {
            case array_key_exists('attestationObject', $response):
                \Akeeba\Passwordless\Assert\Akeeba\Passwordless\Assertion::string($response['attestationObject'], 'Invalid data. The parameter "attestationObject   " is invalid');
            ...
            case array_key_exists('authenticatorData', $response) && array_key_exists('signature', $response):
                $authData = \Akeeba\Passwordless\Base64Url\Akeeba\Passwordless\Base64Url::decode($response['authenticatorData']);
                ...
                return new \Akeeba\Passwordless\Webauthn\AuthenticatorAssertionResponse(
                    ...
                    $authenticatorData,
                    \Akeeba\Passwordless\Base64Url\Akeeba\Passwordless\Base64Url::decode($response['signature']),
