...
use Akeeba\Passwordless\Brick\Math\BigInteger as BrickBigInteger;
use function chr;
...
class BigInteger
{
    ...
    private function __construct(\Akeeba\Passwordless\Brick\Math\BigInteger $value)
    {
        $this->value = $value;
    }
    ...
    public static function createFromBinaryString(string $value): self
    {
        ...
        return new self(\Akeeba\Passwordless\Brick\Math\BigInteger::fromBase($data, 16));
    }

    public static function createFromDecimal(int $value): self
    {
        return new self(\Akeeba\Passwordless\Brick\Math\BigInteger::of($value));
    }
    ...
    {
        if ($this->value->isEqualTo(\Akeeba\Passwordless\Brick\Math\BigInteger::zero())) {
            return '';
        }
