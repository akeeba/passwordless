...
use Akeeba\Passwordless\Ramsey\Uuid\Math\RoundingMode;
 ...
 * @psalm-immutable
 */
class GenericTimeConverter implements \Akeeba\Passwordless\Ramsey\Uuid\Converter\TimeConverterInterface
{
    /**
    ...
    public function __construct(\Akeeba\Passwordless\Ramsey\Uuid\Math\CalculatorInterface $calculator)
    {
    ...
    {
        $timestamp = new \Akeeba\Passwordless\Ramsey\Uuid\Type\Time($seconds, $microseconds);

        // Convert the seconds into a count of 100-nanosecond intervals.
        $sec = $this->calculator->multiply(
            $timestamp->getSeconds(),
            new \Akeeba\Passwordless\Ramsey\Uuid\Type\Integer(self::SECOND_INTERVALS)
        );

        // Convert the microseconds into a count of 100-nanosecond intervals.
        $usec = $this->calculator->multiply(
            $timestamp->getMicroseconds(),
            new \Akeeba\Passwordless\Ramsey\Uuid\Type\Integer(self::MICROSECOND_INTERVALS)
        );
        ...
        $uuidTime = $this->calculator->add(
            $sec,
            $usec,
            new \Akeeba\Passwordless\Ramsey\Uuid\Type\Integer(self::GREGORIAN_TO_UNIX_INTERVALS)
        );
        ...
        return new \Akeeba\Passwordless\Ramsey\Uuid\Type\Hexadecimal($uuidTimeHex);
    }
    ...
    public function convertTime(\Akeeba\Passwordless\Ramsey\Uuid\Type\Hexadecimal $uuidTimestamp): \Akeeba\Passwordless\Ramsey\Uuid\Type\Time
    {
        ...
        $unixTimestamp = $this->calculator->divide(
            \Akeeba\Passwordless\Ramsey\Uuid\Math\RoundingMode::HALF_UP,
            6,
            $epochNanoseconds,
            new \Akeeba\Passwordless\Ramsey\Uuid\Type\Integer(self::SECOND_INTERVALS)
        );
        ...
        return new \Akeeba\Passwordless\Ramsey\Uuid\Type\Time($split[0], $split[1] ?? 0);
    }
}
