 ...
 * @psalm-immutable
 */
class PhpTimeConverter implements \Akeeba\Passwordless\Ramsey\Uuid\Converter\TimeConverterInterface
{
    /**
    ...
    public function __construct(
        ?\Akeeba\Passwordless\Ramsey\Uuid\Math\CalculatorInterface $calculator = null,
        ?\Akeeba\Passwordless\Ramsey\Uuid\Converter\TimeConverterInterface $fallbackConverter = null
    ) {
        if ($calculator === null) {
            $calculator = new \Akeeba\Passwordless\Ramsey\Uuid\Math\BrickMathCalculator();
        }

        if ($fallbackConverter === null) {
            $fallbackConverter = new \Akeeba\Passwordless\Ramsey\Uuid\Converter\Time\GenericTimeConverter($calculator);
        }
    ...
    {
        $seconds = new \Akeeba\Passwordless\Ramsey\Uuid\Type\Integer($seconds);
        $microseconds = new \Akeeba\Passwordless\Ramsey\Uuid\Type\Integer($microseconds);
        ...
        return new \Akeeba\Passwordless\Ramsey\Uuid\Type\Hexadecimal(str_pad(dechex($uuidTime), 16, '0', STR_PAD_LEFT));
    }
    ...
    public function convertTime(\Akeeba\Passwordless\Ramsey\Uuid\Type\Hexadecimal $uuidTimestamp): \Akeeba\Passwordless\Ramsey\Uuid\Type\Time
    {
        ...
        return new \Akeeba\Passwordless\Ramsey\Uuid\Type\Time($splitTime['sec'], $splitTime['usec']);
    }
