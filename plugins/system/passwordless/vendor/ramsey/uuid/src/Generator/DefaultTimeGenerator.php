...
use Akeeba\Passwordless\Ramsey\Uuid\Exception\RandomSourceException;
use Akeeba\Passwordless\Ramsey\Uuid\Exception\TimeSourceException;
 ...
 */
class DefaultTimeGenerator implements \Akeeba\Passwordless\Ramsey\Uuid\Generator\TimeGeneratorInterface
{
    /**
    ...
    public function generate($node = null, ?int $clockSeq = null): string
    {
        ...
        if ($clockSeq === null) {
            try {
                ...
                throw new \Akeeba\Passwordless\Ramsey\Uuid\Exception\RandomSourceException(
        ...
        if (strlen($timeHex) !== 16) {
            throw new \Akeeba\Passwordless\Ramsey\Uuid\Exception\TimeSourceException(sprintf(
    ...
    private function getValidNode($node): string
    {
        ...
        if (!ctype_xdigit((string) $node) || strlen((string) $node) > 12) {
            throw new \Akeeba\Passwordless\Ramsey\Uuid\Exception\InvalidArgumentException('Invalid node value');
        }
