...
use Akeeba\Passwordless\Safe\Exceptions\SocketsException;
use const PREG_NO_ERROR;
use Akeeba\Passwordless\Safe\Exceptions\ApcException;
use Akeeba\Passwordless\Safe\Exceptions\ApcuException;
use Akeeba\Passwordless\Safe\Exceptions\JsonException;
use Akeeba\Passwordless\Safe\Exceptions\OpensslException;
use Akeeba\Passwordless\Safe\Exceptions\PcreException;
...
function json_decode(string $json, bool $assoc = false, int $depth = 512, int $options = 0)
{
    ...
    if (JSON_ERROR_NONE !== json_last_error()) {
        throw \Akeeba\Passwordless\Safe\Exceptions\JsonException::createFromPhpError();
    }
    return $data;
}
...
function apc_fetch($key)
{
    ...
    if ($success === false) {
        throw \Akeeba\Passwordless\Safe\Exceptions\ApcException::createFromPhpError();
    }
    return $result;
}
...
function apcu_fetch($key)
{
    ...
    if ($success === false) {
        throw \Akeeba\Passwordless\Safe\Exceptions\ApcuException::createFromPhpError();
    }
    return $result;
}
...
function preg_replace($pattern, $replacement, $subject, int $limit = -1, int &$count = null)
{
    ...
    if (preg_last_error() !== PREG_NO_ERROR || $result === null) {
        throw \Akeeba\Passwordless\Safe\Exceptions\PcreException::createFromPhpError();
    }
    return $result;
}
...
function openssl_encrypt(string $data, string $method, string $key, int $options = 0, string $iv = "", string &$tag = "", string $aad = "", int $tag_length = 16): string
{
    ...
    if ($result === false) {
        throw \Akeeba\Passwordless\Safe\Exceptions\OpensslException::createFromPhpError();
    }
    return $result;
}
...
function socket_write($socket, string $buffer, int $length = 0): int
{
    ...
    if ($result === false) {
        throw \Akeeba\Passwordless\Safe\Exceptions\SocketsException::createFromPhpError();
    }
    return $result;
}
