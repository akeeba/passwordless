...
class Integer extends \Akeeba\Passwordless\FG\ASN1\ASNObject implements \Akeeba\Passwordless\FG\ASN1\Parsable
{
    /** @var int */
    private $value;
    ...
    {
        return \Akeeba\Passwordless\FG\ASN1\Identifier::INTEGER;
    }
    ...
    {
        $value = \Akeeba\Passwordless\FG\Utility\BigInteger::create($this->value, 10);
    ...
    private static function ensureMinimalEncoding($binaryData, $offsetIndex)
    {
        ...
        if ((ord($binaryData[$offsetIndex]) == 0x00 && (ord($binaryData[$offsetIndex+1]) & 0x80) == 0) ||
            ...
            throw new \Akeeba\Passwordless\FG\ASN1\Exception\ParserException("Integer not minimally encoded", $offsetIndex);
        }
    }
    ...
    public static function fromBinary(&$binaryData, &$offsetIndex = 0)
    {
        ...
        $number = \Akeeba\Passwordless\FG\Utility\BigInteger::create(ord($binaryData[$offsetIndex++]) & 0x7F);
        ...
        if ($isNegative) {
            $number = $number->subtract(\Akeeba\Passwordless\FG\Utility\BigInteger::create(2)->toPower(8 * $contentLength - 1));
        }
