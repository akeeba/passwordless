...
class RelativeObjectIdentifier extends \Akeeba\Passwordless\FG\ASN1\Universal\ObjectIdentifier implements \Akeeba\Passwordless\FG\ASN1\Parsable
{
    ...
    {
        return \Akeeba\Passwordless\FG\ASN1\Identifier::RELATIVE_OID;
    }

    public static function fromBinary(&$binaryData, &$offsetIndex = 0)
    {
        self::parseIdentifier($binaryData[$offsetIndex], \Akeeba\Passwordless\FG\ASN1\Identifier::RELATIVE_OID, $offsetIndex++);
        $contentLength = self::parseContentLength($binaryData, $offsetIndex, 1);

        try {
            $oidString = self::parseOid($binaryData, $offsetIndex, $contentLength);
        } catch (\Akeeba\Passwordless\FG\ASN1\Exception\ParserException $e) {
            throw new \Akeeba\Passwordless\FG\ASN1\Exception\ParserException('Malformed ASN.1 Relative Object Identifier', $e->getOffset());
        }
